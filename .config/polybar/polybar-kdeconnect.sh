...
COLOR_DISCONNECTED='#DFDFDF'       # Device Disconnected
COLOR_NEWDEVICE='#DFDFDF'          # New Device
COLOR_BATTERY_90='#DFDFDF'         # Battery >= 90
COLOR_BATTERY_80='#DFDFDF'         # Battery >= 80
COLOR_BATTERY_70='#DFDFDF'         # Battery >= 70
COLOR_BATTERY_60='#DFDFDF'         # Battery >= 60
COLOR_BATTERY_50='#DFDFDF'         # Battery >= 50
COLOR_BATTERY_LOW='#DFDFDF'        # Battery <  50

# Icons shown in Polybar
ICON_SMARTPHONE='phone'
ICON_TABLET='tablet'
SEPERATOR='|'
...
show_pmenu () {
    menu="$(rofi -sep "|" -dmenu -i -p "$DEV_NAME" -location $LOCATION -yoffset $YOFFSET -xoffset $XOFFSET -theme -width $WIDTH -hide-scrollbar -line-padding 1 -padding 20 -lines 1<<<"Pair Device")"
                case "$menu" in
...
show_pmenu2 () {
    menu="$(rofi -sep "|" -dmenu -i -p "$1 has sent a pairing request" -location $LOCATION -yoffset $YOFFSET -xoffset $XOFFSET -theme -width $WIDTH_WIDE -hide-scrollbar -line-padding 4 -padding 20 -lines 2 <<< "Accept|Reject")"
                case "$menu" in
...
get_icon () {
    ...
    case $1 in
    "-1")     ICON="%{F$COLOR_DISCONNECTED}$icon%{F-} disconnected" ;;
    "-2")     ICON="%{F$COLOR_NEWDEVICE}$icon%{F-} new" ;;
    5*)     ICON="%{F$COLOR_BATTERY_50}$icon%{F-} connected" ;;
    6*)     ICON="%{F$COLOR_BATTERY_60}$icon%{F-} connected" ;;
    7*)     ICON="%{F$COLOR_BATTERY_70}$icon%{F-} connected" ;;
    8*)     ICON="%{F$COLOR_BATTERY_80}$icon%{F-} connected" ;;
    9*|100) ICON="%{F$COLOR_BATTERY_90}$icon%{F-} connected" ;;
    *)      ICON="%{F$COLOR_BATTERY_LOW}$icon%{F-} connected" ;;
    esac
    echo $ICON
}
